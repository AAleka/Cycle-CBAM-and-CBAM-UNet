...
def save_models(g_model_AtoB, iteration):
    filename1 = f'results/models/g_model_ArttoNoArt{iteration}.h5'
...
def summarize_performance(step, g_model, trainX, name, n_samples=5):
    ...
    filename1 = 'results/training/%s_generated_plot_%06d.png' % (name, step)
...
def save_generated(step, g_model, trainX, n_samples=1):
    ...
    pyplot.savefig(f"results/Original bad/{step}_real.png")

    pyplot.imshow(X_out[0])
    pyplot.savefig(f"results/Original bad/{step}_fake.png")
...
def validate(g_model_AtoB):
    file = open('results/validation.csv', 'r')
...
def plot_metrics(all_psnr, all_ssim, flag="training"):
    if flag == "training":
        ...
        pyplot.xlabel("Steps")
        pyplot.savefig(f"results/training_PSNR.png")
        pyplot.close()

        pyplot.plot(np.arange(0, len(all_ssim), 1, dtype=int), all_ssim)
        pyplot.ylabel("Average SSIM value")
        pyplot.xlabel("Steps")
        pyplot.savefig(f"results/training_SSIM.png")
        pyplot.close()
    elif flag == "validation":
        pyplot.plot(np.arange(0, len(all_psnr), 1, dtype=int), all_psnr)
        pyplot.ylabel("Average PSNR value")
        pyplot.xlabel("Steps")
        pyplot.savefig(f"results/validation_PSNR.png")
        pyplot.close()

        pyplot.plot(np.arange(0, len(all_ssim), 1, dtype=int), all_ssim)
        pyplot.ylabel("Average SSIM value")
        pyplot.xlabel("Steps")
        pyplot.savefig(f"results/validation_SSIM.png")
        pyplot.close()
