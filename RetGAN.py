...
os.mkdir(f'retinal_model')
os.mkdir(f'retinal_model/models')
os.mkdir(f'retinal_model/Original bad')
os.mkdir(f'retinal_model/testing')
os.mkdir(f'retinal_model/training')
...
# dataLQ_train, test_path = load_images(path1 + '1/', path2 + 'artifacts/')
...
test_df.to_csv('retinal_model/test.csv', index=False)

random.shuffle(valid_path)
valid_df = pd.DataFrame(valid_path)
valid_df.to_csv('retinal_model/validation.csv', index=False)

del test_path, valid_path, test_df, valid_df
gc.collect()

dataHQ_all = load_images(path1 + '0/', path2 + 'no_artifacts/', flag='high')
